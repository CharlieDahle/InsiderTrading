 ...
 */
 class person {
  ...
  /**
   * clears all current transactions from the current table
   */
  ...
  /**
   * clears all current users from the table
   */
  function clearUsers() {
    usersInGroup = [];
  }
  ...
  /**
   * clears the current calculated answer
   */
  ...
  function changeMainTab(tabId) {
    ...
    if (tabId.startsWith("home_table")) {
      ...
      document.getElementById("table_title").innerHTML = tableId.replace("_", " ") + " group";
  ...
  function addCircleWithName(name) {
    ...
    parentDiv.onclick = function(){ changeMainTab('home_table_'+this.circle_name)};
  }
  ...
  function addTabToHomeWithName(name) {
    ...
    newTabText.onclick = function(){ changeMainTab('home_table_'+this.circle_name)};
  }
  ...
  function loadToTable(tableId) {
    ...
    if (tableRaw != null) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        g.makePayment(correctPerson, transaction.total, transaction.item, transaction.date);
      }
  ...
  function add_transaction(){
    ...
    if(!usersInGroup.includes(name)){
      usersInGroup.push(name);
    }
  
  
    if(name.trim() != "" && amount.trim() != "" && item.trim() != "" && date.trim() != ""){
      g.makePayment(new person(name), amount, item, date);
    
  
      saveCurrentTable(window.currentTableId);
